...
public:
    ...
    CreateBody toGroundBody(ToGroundArg &x, Ground::UStmVec &stms, NAF naf, UTerm &&id, std::shared_ptr<Ground::TheoryLiteral*> lit) const;
    CreateHead toGroundHead(std::shared_ptr<Ground::TheoryLiteral*> sharedLit) const;
...
public:
    BodyTheoryLiteral(NAF naf, TheoryAtom &&atom);
    virtual ~BodyTheoryLiteral() noexcept;
    Ground::TheoryLiteral &theoryLit() { return **sharedLit_; }
    std::shared_ptr<Ground::TheoryLiteral*> sharedLit() {
        if (!sharedLit_) { sharedLit_ = std::make_shared<Ground::TheoryLiteral*>(nullptr); }
        return sharedLit_;
    }
...
private:
    std::shared_ptr<Ground::TheoryLiteral*> sharedLit_;
    TheoryAtom atom_;
    NAF naf_;
};
...
public:
    HeadTheoryLiteral(TheoryAtom &&atom);

...
LiteralId TheoryLiteral::translate(Translator &trans) {
    ...
    if (!atm.translated()) {
        ...
        if (atm.defined()) {
            ...
            Potassco::TheoryAtom::Occurrence occ = atm.type() == TheoryAtomType::Body ? Potassco::TheoryAtom::Occurrence::occ_body : Potassco::TheoryAtom::Occurrence::occ_head;
            auto newAtom = [&]() -> Atom_t {
            ...
            auto ret = atm.hasGuard()
                ? data.addAtom(newAtom, occ, atm.name(), Potassco::toSpan(atm.elems()), atm.op(), atm.guard())
                : data.addAtom(newAtom, occ, atm.name(), Potassco::toSpan(atm.elems()));
...
LiteralId DisjunctionLiteral::translate(Translator &x) {
    ...
    if (!atm.translated()) {
        ...
        atm.simplify();
        if (atm.headFact()) { return atm.lit(); }
        Rule dj;
        dj.addBody(atm.lit());
...
Gringo::TheoryData::TermType DomainData::termType(Id_t value) const {
    ...
    switch (term.type()) {
        ...
        case Potassco::Theory_t::Compound: {
            ...
            switch (term.tuple()) {
                case Potassco::TupleType::Paren:   { return Gringo::TheoryData::TermType::Tuple; }
                case Potassco::TupleType::Bracket: { return Gringo::TheoryData::TermType::List; }
                case Potassco::TupleType::Brace:   { return Gringo::TheoryData::TermType::Set; }
            }
...
Gringo::TheoryData::AtomType DomainData::atomType(Id_t value) const {
    auto &atom = theory_.getAtom(value);
    if (atom.atom() == 0) {
        return Gringo::TheoryData::AtomType::Directive;
    }
    else if (atom.occurrence() == Potassco::TheoryAtom::occ_head) {
        return Gringo::TheoryData::AtomType::Head;
    }
    else {
        return Gringo::TheoryData::AtomType::Body;
    }
}

...
bool termEqual(Potassco::TheoryTerm const &term, Potassco::TupleType type, Potassco::IdSpan const &terms) {
...
bool atomEqual(Potassco::TheoryAtom const &a, Potassco::TheoryAtom::Occurrence occ, Potassco::Id_t termId, Potassco::IdSpan const &elems) {
    return !a.guard() && a.occurrence() == occ && a.term() == termId && a.size() == elems.size && std::equal(a.begin(), a.end(), Potassco::begin(elems));
}

bool atomEqual(Potassco::TheoryAtom const &a, Potassco::TheoryAtom::Occurrence occ, Potassco::Id_t termId, Potassco::IdSpan const &elems, Potassco::Id_t guard, Potassco::Id_t rhs) {
    return a.guard() && *a.guard() == guard && *a.rhs() == rhs && a.occurrence() == occ && a.term() == termId && a.size() == elems.size && std::equal(a.begin(), a.end(), Potassco::begin(elems));
}

bool atomEqual(Potassco::TheoryAtom const &a, Potassco::TheoryAtom const &b) {
    return b.guard()
        ? atomEqual(a, b.occurrence(), b.term(), b.elements(), *b.guard(), *b.rhs())
        : atomEqual(a, b.occurrence(), b.term(), b.elements());
}
...
size_t termHash(Potassco::TupleType type, Potassco::IdSpan const &terms) {
...
size_t atomHash(Potassco::TheoryAtom::Occurrence occ, Potassco::Id_t termId, Potassco::IdSpan const &elems) {
    size_t seed = occ;
...
size_t atomHash(Potassco::TheoryAtom::Occurrence occ, Potassco::Id_t termId, Potassco::IdSpan const &elems, Potassco::Id_t guard, Potassco::Id_t rhs) {
    size_t seed = atomHash(occ, termId, elems);
...
size_t atomHash(Potassco::TheoryAtom const &atom) {
    return atom.guard()
        ? atomHash(atom.occurrence(), atom.term(), atom.elements(), *atom.guard(), *atom.rhs())
        : atomHash(atom.occurrence(), atom.term(), atom.elements());
}
...
void TupleTheoryTerm::print(std::ostream &out) const {
    switch (type_) {
        case Potassco::Tuple_t::Paren:   { out << "("; break; }
        case Potassco::Tuple_t::Brace:   { out << "{"; break; }
        case Potassco::Tuple_t::Bracket: { out << "["; break; }
    }
...
Potassco::Id_t TheoryData::addTerm(Potassco::TupleType type, Potassco::IdSpan const& terms) {
...
Potassco::Id_t TheoryData::addTerm(Value value) {
    switch (value.type()) {
        ...
        case Value::FUNC: {
            ...
            if (value.name()->empty()) {
                return addTerm(Potassco::TupleType::Paren, Potassco::toSpan(args));
            }
            else {
...
std::pair<Potassco::TheoryAtom const &, bool> TheoryData::addAtom(std::function<Potassco::Id_t()> newAtom, Potassco::TheoryAtom::Occurrence occ, Potassco::Id_t termId, Potassco::IdSpan const &elems) {
    return addAtom_(newAtom, occ, termId, elems);
}

std::pair<Potassco::TheoryAtom const &, bool> TheoryData::addAtom(std::function<Potassco::Id_t()> newAtom, Potassco::TheoryAtom::Occurrence occ, Potassco::Id_t termId, Potassco::IdSpan const &elems, Potassco::Id_t op, Potassco::Id_t rhs) {
    return addAtom_(newAtom, occ, termId, elems, op, rhs);
}
...
void TheoryData::printTerm(std::ostream &out, Potassco::Id_t termId) const {
    ...
    switch (term.type()) {
        ...
        case Potassco::Theory_t::Compound: {
            auto &parens = Potassco::Tuple_t::parens(term.isTuple() ? term.tuple() : Potassco::Tuple_t::Paren);
            bool isOp = false;
            if (term.isFunction()) {
            ...
            if (term.isTuple() && term.tuple() == Potassco::TupleType::Paren && term.size() == 1) { out << ","; }
            out << parens.second;
            break;
        }
    }

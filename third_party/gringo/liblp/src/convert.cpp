...
HeadView SmodelsConvert::SmData::mapHead(const HeadView& h) {
	...
	HeadView r = {h.type, toSpan(head_)};
	return r;
}
...
// SmodelsConvert
/////////////////////////////////////////////////////////////////////////////////////////
SmodelsConvert::SmodelsConvert(LpElement& out, bool ext) : out_(out), data_(new SmData), ext_(ext) {}
...
void SmodelsConvert::rule(const HeadView& head, const BodyView& body) {
	...
	if (size(mHead) == 0) {
		if (mHead.type == Head_t::Choice) return;
		Atom_t f = data_->falseAtom();
		mHead.atoms = toSpan(&f, 1);
	}
	...
	HeadView auxHead = {Head_t::Disjunctive, toSpan(&aux, 1)};
	out_.rule(auxHead, mBody);
	WeightLit_t auxLit = {static_cast<Lit_t>(aux), 1};
	BodyView auxBody = {Body_t::Normal, 0, toSpan(&auxLit, 1)};
	out_.rule(mHead, auxBody);
}
...
void SmodelsConvert::flushMinimize() {
	for (SmData::MinMap::iterator it = data_->minimize_.begin(), end = data_->minimize_.end(); it != end; ++it) {
		BodyView body = {Body_t::Sum, 0, toSpan(it->second)};
		BodyView mBody = data_->mapBody(body);
		out_.minimize(it->first, mBody.lits);
	}
}
void SmodelsConvert::flushExternal() {
	for (SmData::AtomVec::const_iterator it = data_->extern_.begin(), end = data_->extern_.end(); it != end; ++it) {
		...
		if (!ext_) {
			Atom_t at = a;
			HeadView h = {Head_t::Disjunctive, toSpan(&at, 1)};
			BodyView b = {Body_t::Normal, 0, toSpan<WeightLit_t>()};
			if (!a.head && vt == Value_t::True) {
				out_.rule(h, b);
			}
			else if (!a.head && vt == Value_t::Free) {
				h.type = Head_t::Choice;
				out_.rule(h, b);
			}
		}
		else {

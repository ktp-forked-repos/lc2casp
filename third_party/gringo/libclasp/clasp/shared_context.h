...
struct VarInfo {
	...
	VarInfo(VarType t = Var_t::Atom) : rep(0) {
		if      (t == Var_t::Body)  { set(VarInfo::BODY); }
		else if (t == Var_t::Hybrid){ set(VarInfo::EQ); }
	}
...
public:
	...
	//! Adds a range of output variables.
	/*!
	 * \note Ranges must be increasing and shall not overlap.
	 */
	void add(const RangeType& r);
	...
	range_iterator vars_begin() const { return vars_.begin(); }
	range_iterator vars_end()   const { return vars_.end(); }
	...
	uint32 numRange() const { return static_cast<uint32>(vars_.size());  }
...
public:
   ...
   */
	...
	void    resizeVars(uint32 maxVar) { varInfo_.resize(std::max(Var(1), maxVar)); stats_.vars.num = numVars(); }
	//! Adds a new variable.
	/*!
	 * \param inf Information associated with the new variable.
	 * \note Problem variables are numbered from 1 onwards!
	 */
	Var     addVar(const VarInfo& inf);

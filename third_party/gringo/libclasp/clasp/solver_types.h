...
#define CLASP_EXTENDED_STATS(STAT, SELF, OTHER)     \
	...
	void addLearnt(uint32 size, type_t t) {
		assert(t != Constraint_t::Static && t <= Constraint_t::Type__max);
		learnts[t-1]+= 1;
		lits[t-1]   += size;
		binary      += (size == 2);
		ternary     += (size == 3);
...
public:
	...
	ClauseInfo(ConstraintType t = Constraint_t::Static) : act_(0), lbd_(MAX_LBD), type_(t), tag_(0), aux_(0) {
...
protected:
	...
	bool         hasLbd() const  { return info_.data.type != Constraint_t::Other || lbd() != MAX_LBD; }
...
struct CCMinRecursive {
	...
	bool  checkRecursive(Literal p) {
		if (state(p) == state_open) { dfsStack.push_back(p.unflag()); }

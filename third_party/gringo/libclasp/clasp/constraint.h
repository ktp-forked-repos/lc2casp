...
public:
	...
	/*!
	 * \note The default implementation returns Constraint_t::Static.
	 */
...
public:
	...
	/*!
	 * \note The default implementation returns Constraint_t::Conflict.
	 */
	...
	bool minimize(S& s, Literal p, CCMinRecursive* rec) const {
		...
		if (t == Generic) {
			...
			return s.ccMinimize(firstLiteral(), rec)
				&& (t != Ternary || s.ccMinimize(secondLiteral(), rec));
		}
	}

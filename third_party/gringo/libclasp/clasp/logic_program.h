...
struct RuleStats {
	enum Key {
		...
		Minimize  = Potassco::Directive_t::Minimize, /**< Distinct minimize constraints. */
...
public:
	...
	struct AspOptions {
		...
		AspOptions() : erMode(mode_native), iters(5), noSCC(0), suppMod(0), dfOrder(0), backprop(0), oldMap(0), noGamma(0) {}
	...
	//! Calls the given visitor on the parts of the simplified program.
	void accept(LogicProgramVisitor& out);
	...
	LogicProgram& addOutput(const ConstString& str, Potassco::Lit_t cond) {
		return addOutput(str, Potassco::toSpan(&cond, 1));
	}
	...
	LogicProgram& addAcycEdge(uint32 n1, uint32 n2, const Potassco::LitSpan& condition);
	LogicProgram& addAcycEdge(uint32 n1, uint32 n2, Potassco::Lit_t cond) {
		return addAcycEdge(n1, n2, Potassco::toSpan(&cond, 1));
	}

	//! Adds a conditional domain heuristic directive to the program.
	LogicProgram& addDomHeuristic(Atom_t atom, DomModType t, int bias, unsigned prio, const Potassco::LitSpan& condition);
	LogicProgram& addDomHeuristic(Atom_t atom, DomModType t, int bias, unsigned prio, Potassco::Lit_t cond) {
		return addDomHeuristic(atom, t, bias, prio, Potassco::toSpan(&cond, 1));
	}
	...
	Atom_t startAtom()       const { return incData_?incData_->atoms.lo:1; }
...
private:
	...
	struct Incremental  {
		...
		explicit Incremental(uint32 n);
		AtomRange atoms;        // atoms of current step
...
//! Interface for visiting a logic program.
class LogicProgramVisitor : public Potassco::LpElement {
public:
	virtual ~LogicProgramVisitor();
	virtual void theory(const TheoryData& t) = 0;
};

...
OPTION(opt_strategy , ""  , ARG(arg("<arg>")->implicit("1"), DEFINE_ENUM_MAPPING(MinimizeMode_t::Strategy,\
       ...
       , FUN(str) { ARG_T(MinimizeMode_t::Strategy, uint32) arg(MinimizeMode_t::opt_bb, 0); uint32 n;\
         return ITE(stringTo(str, arg), SET(SELF.optStrat, (uint32)arg.first) && SET(SELF.optParam, arg.second) && (arg.first != 0 || arg.second < 4),\
                    stringTo(str, n) && SET(SELF.optStrat, uint32(n>3)) && SET(SELF.optParam, (n > 3 ? n - 4:n)));}\
...
OPTION(heuristic, "", ARG(arg("<heu>"), DEFINE_ENUM_MAPPING(Heuristic_t::Type, \
       ...
       "        None   : Select the first free variable", FUN(str) { ARG_T(Heuristic_t::Type, uint32) arg(Heuristic_t::Default,0); \
       return stringTo(str, arg) && SET(SELF.heuId, (uint32)arg.first) && (Heuristic_t::isLookback(arg.first) || !arg.second) && SET_OR_FILL(SELF.heuristic.param, arg.second);},\
       toString((Heuristic_t::Type)SELF.heuId, SELF.heuristic.param))
OPTION(init_moms  , "!,@2", ARG(flag())    , "Initialize heuristic with MOMS-score", STORE_FLAG(SELF.heuristic.moms), toString(SELF.heuristic.moms))
OPTION(score_res  , ",@2" , ARG(arg("<n>")), "Resolution score {0=default|1=min|2=set|3=multiset}", STORE_LEQ(SELF.heuristic.score,3u), toString(SELF.heuristic.score))
OPTION(score_other, ",@2" , ARG(arg("<n>")), "Score {0=no|1=loop|2=all} other learnt nogoods", STORE_LEQ(SELF.heuristic.other,2u), toString(SELF.heuristic.other))
...
OPTION(berk_huang , "!,@2", ARG(flag())    , "Enable/Disable Huang-scoring in Berkmin", STORE_FLAG(SELF.heuristic.huang), toString(SELF.heuristic.huang))
OPTION(vsids_acids, "!,@2", ARG(flag())    , "Enable/Disable acids-scheme in Vsids/Domain", STORE_FLAG(SELF.heuristic.acids), toString(SELF.heuristic.acids))
OPTION(nant       , "!,@2", ARG(flag())    , "In Unit count only atoms in NAnt(P)", STORE_FLAG(SELF.heuristic.nant), toString(SELF.heuristic.nant))
OPTION(dom_mod    , ",@2" , NO_ARG         , "Default modification for domain heuristic\n"\
       ...
       FUN(str) { ARG_T(uint32, uint32) arg(0,0); return stringTo(str, arg) && SET_LEQ(SELF.heuristic.domMod, arg.first, 7u) && SET(SELF.heuristic.domPref, arg.second);},\
       toString(SELF.heuristic.domMod, SELF.heuristic.domPref))
...
OPTION(acyc_prop, ",@2", ARG(implicit("1")->arg("{0..1}")), "Acyc propagate with {0=inc|1=inc+back}", \
       FUN(str) { uint32 x; return stringTo(str, x) && SET_LEQ(SELF.acycFwd, (1u-x), 1u); }, \
       toString(1u-SELF.acycFwd))
...
OPTION(loops, "" , ARG(arg("<type>"), DEFINE_ENUM_MAPPING(DefaultUnfoundedCheck::ReasonStrategy,\
       ...
       "        no      : Do not learn loop formulas\n", FUN(str) {DefaultUnfoundedCheck::ReasonStrategy arg; return stringTo(str, arg) && SET(SELF.loopRep, (uint32)arg);},\
...
OPTION(partial_check, "", ARG(implicit("50")), "Configure partial stability tests\n" \
       ...
       ARG_T(uint32, uint32) arg(0,0);\
       return stringTo(str, arg | off) && (arg.parsed() < 2 || arg.first != 0u) && SET_LEQ(SELF.fwdCheck.highPct, arg.first, 100u) && SET_OR_ZERO(SELF.fwdCheck.highStep, arg.second);},\
       TO_STR_IF(SELF.fwdCheck.highPct, SELF.fwdCheck.highPct, SELF.fwdCheck.highStep))
...
OPTION(restarts, "!,r", ARG(arg("<sched>")), "Configure restart policy\n" \
       ...
       stringTo(str, SELF.sched) && SET(SELF.dynRestart, uint32(SELF.sched.type == ScheduleStrategy::User)));}, toString(SELF.sched))
...
OPTION(del_grow    , "!", NO_ARG, "Configure size-based deletion policy\n" \
       ...
       return stringTo(str, off | arg) && ITE(arg.empty(), (SELF.growSched = ScheduleStrategy::none(), SELF.fGrow = 0.0f, true),\
         (arg.third.defaulted() || arg.third.type != ScheduleStrategy::User) && SET_R(SELF.fGrow, (float)arg.first, 1.0f, FLT_MAX) && SET_R(SELF.fMax, (float)arg.second, 0.0f, FLT_MAX) && (SELF.growSched = arg.third, true));\
...
OPTION(del_cfl     , "!", ARG(arg("<sched>")), "Configure conflict-based deletion policy\n" \
       ...
       return ITE(stringTo(str, off), (SELF.cflSched=ScheduleStrategy::none()).disabled(), stringTo(str, SELF.cflSched) && SELF.cflSched.type != ScheduleStrategy::User);}, toString(SELF.cflSched))
...
GROUP_BEGIN(SELF)
OPTION(supp_models, ",@1" , ARG(flag())    , "Compute supported models", STORE_FLAG(SELF.suppMod), toString(SELF.suppMod))
OPTION(no_ufs_check, ",@1" , ARG(flag())   , "Disable unfounded set check", STORE_FLAG(SELF.noSCC), toString(SELF.noSCC))
...
OPTION(dlp_old_map, ",@3" , ARG(flag())    , "Enable old mapping for disjunctive LPs", STORE_FLAG(SELF.oldMap), toString(SELF.oldMap))
...
OPTION(global_restarts, ",@1", ARG(implicit("5")->arg("<X>")), "Configure global restart policy\n" \
       ...
       return stringTo(str, arg | off) && (arg.parsed() < 2 || arg.first != 0u) && (SELF.restarts.sched=arg.second).type != ScheduleStrategy::User && SET(SELF.restarts.maxR, arg.first);},\

...
public:
	...
	PP(TheoryPropagator& w) : wrapper_(&w), init_(0), delta_(0), isModel_(false) {
...
bool TheoryPropagator::PP::isModel(Solver& s) {
	...
	struct Scoped {
		~Scoped() { *b = false; }
		Potassco::AbstractPropagator* prop;
		bool* b;
	} guard = { wrapper_->prop_, &(isModel_ = true) };
	return guard.prop->model(*this) && !s.hasConflict();
}
...
bool TheoryPropagator::PP::addClause(const Potassco::LitSpan& clause) {
	...
	if (isModel_ || impLevel >= solver_->decisionLevel()) {
		bool ok = !isModel_ || (status_ == ClauseCreator::status_sat);
		status_ = ClauseCreator::status_open;
		return ClauseCreator::create(*solver_, rep, ccFlags_s) && ok;
	}
	return false;
}
bool TheoryPropagator::PP::propagate() {
	if (solver_->hasConflict()) { return false; }
	return !isModel_ ? solver_->propagateUntil(this) : solver_->numFreeVars() == 0 && solver_->queueSize() == 0;
}
/////////////////////////////////////////////////////////////////////////////////////////
// TheoryPropagator
/////////////////////////////////////////////////////////////////////////////////////////

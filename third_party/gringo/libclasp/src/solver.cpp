...
bool Solver::endStep(uint32 top, const SolverParams& params) {
	...
	if (value(x.var()) == value_free){ force(~x, lit_true());}
	post_.disable();
	if (simplify()) {
...
void Solver::popAuxVar(uint32 num) {
	...
	LitVec cc;
	ConstraintDB* auxCons = allocUndo(0);
	auxCons->clear();
	for (i = j = 0; i != end; ++i) {
		cc.clear();

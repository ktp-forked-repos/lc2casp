...
AcyclicityCheck::AcyclicityCheck(DependencyGraph* graph) : graph_(graph), solver_(0), nogoods_(0), prop_(prop_full), tagCnt_(0), genId_(0)  {}
AcyclicityCheck::~AcyclicityCheck() { 
	delete nogoods_; 
}

void AcyclicityCheck::setPropagationStrategy(PropagationStrategy p) { 
	prop_ = p; 
}
...
bool AcyclicityCheck::propagateFixpoint(Solver& s, PostPropagator*) {
	for (Arc x; !todo_.empty();) {
		x = todo_.pop_ret();
		if (!dfsForward(s, x) || (prop_ != prop_fwd && !dfsBackward(s, x))) {
			return false;
		}
	}
	todo_.clear();
	...
	for (Var node, next; !nStack_.empty(); ) {
		...
		for (const Inv* a = graph_->invBegin(node); a; a = graph_->invNext(a)) {
			...
			if (visited(next, fwd)) { // a->lit would complete a cycle - force to false
				...
				if (val == value_free && prop_ != prop_no_learn) {
					ClauseInfo info(Constraint_t::Loop);

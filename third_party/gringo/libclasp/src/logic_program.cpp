//
// Copyright (c) 2013-2015, Benjamin Kaufmann
//
...
LogicProgram::LogicProgram() : theory_(0), auxData_(0), incData_(0) {
	CLASP_FAIL_IF(!init_trueAtom_g, "invalid static init");
	accu = 0;
}
LogicProgram::~LogicProgram() { dispose(true); }
LogicProgram::Incremental::Incremental(uint32 n) : atoms(1, n), startScc(0) {}
...
void LogicProgram::setOptions(const AspOptions& opts) {
	...
	if (opts.suppMod && ctx() && ctx()->sccGraph.get()) {
		ctx()->report(warning(Event::subsystem_facade, "'supp-models' ignored for non-tight programs."));
		opts_.suppMod = 0;
		opts_.noSCC   = 0;
	}
}
...
bool LogicProgram::doUpdateProgram() {
	if (!incData_) { incData_ = new Incremental((uint32)atoms_.size()); }
	...
	// {ai | ai in P}.
	PrgBody* support = incData_->atoms.hi > 1 ? getTrueBody() : 0;
	propQ_.clear();
	for (Atom_t i = 1, end = incData_->atoms.hi; i != end; ++i) {
		if (getRootId(i) >= incData_->atoms.hi) {
	...
	deleteAtoms(incData_->atoms.hi);
	atoms_.erase(atoms_.begin()+incData_->atoms.hi, atoms_.end());
	uint32 nAtoms = (uint32)atoms_.size();
	atomState_.resize(nAtoms);
	incData_->atoms = Incremental::AtomRange(nAtoms, nAtoms);
	stats.reset();
	statsId_ = 0;
	return true;
}
...
void LogicProgram::accept(LogicProgramVisitor& out) {
	Potassco::HeadView head = {Potassco::Head_t::Disjunctive, Potassco::toSpan<Potassco::Atom_t>()};
	Potassco::BodyView body = {Potassco::Body_t::Normal, 0, Potassco::toSpan<Potassco::WeightLit_t>()};
	if (!ok()) {
		out.rule(head, body);
		return;
	}
	...
	for (Atom_t i = startAtom(); i < atoms_.size(); ++i) {
		if (atoms_[i]->eq()) {
			head.atoms = Potassco::toSpan(&i, 1);
			bodyLits[0].lit = Potassco::lit(getRootId(i));
			body.lits = Potassco::toSpan(bodyLits);
			...
			head.atoms = Potassco::toSpan<Potassco::Atom_t>();
			body.lits  = Potassco::toSpan(bodyLits);
	...
	if (theory_ && theory_->numAtoms()) {
		head.atoms = Potassco::toSpan<Potassco::Atom_t>();
		for (Potassco::TheoryData::atom_iterator it = theory_->begin(), end = theory_->end(); it != end; ++it) {
			const Atom_t id = (*it)->atom();
			if (validAtom(id) && atomState_.isSet(id, AtomState::false_flag) && !inProgram(id)) {
				bodyLits[0].lit = Potassco::lit(id);
				out.rule(head, body);
			}
		}
	}
	...
	head.type = Potassco::Head_t::Disjunctive;
		...
		if (b->relevant() && (b->inRule() || b->value() == value_false) && transform(*this, *b, body, bodyLits)) {
			...
			for (PrgBody::head_iterator hIt = b->heads_begin(); hIt != b->heads_end(); ++hIt) {
				...
				if (hIt->isAtom() && hIt->node() && inProgram(hIt->node())) {
					if      (hIt->isNormal()) { head.atoms = Potassco::toSpan(&(normal = hIt->node()), 1); out.rule(head, body); }
					...
					if (simp && getRootAtom(hIt->node())->var() == b->var() && body.type != Potassco::Body_t::Normal) {
						...
						body.type = Potassco::Body_t::Normal;
			...
			if (!choice.empty()) {
				head.type  = Potassco::Head_t::Choice;
				head.atoms = Potassco::toSpan(choice);
				out.rule(head, body);
			}
			head.type = Potassco::Head_t::Disjunctive;
	...
	for (MinList::iterator it = minimize_.begin(), end = minimize_.end(); it != end; ++it) {
		out.minimize((*it)->prio, Potassco::toSpan((*it)->lits));
	}
...
LogicProgram& LogicProgram::addOutput(const ConstString& str, const Potassco::LitSpan& cond) {
	if (!ctx()->output.filter(str)) {
		show_.push_back(ShowPair(cond.size == 1 ? Potassco::id(*cond.first) : newCondition(cond), str));
	}
	return *this;
}
...
LogicProgram& LogicProgram::addAcycEdge(uint32 n1, uint32 n2, const Potassco::LitSpan& cond) {
	AcycArc arc = { newCondition(cond), {n1, n2} };
...
LogicProgram& LogicProgram::addDomHeuristic(Atom_t atom, DomModType type, int bias, unsigned prio, const Potassco::LitSpan& cond) {
...
Literal LogicProgram::getLiteral(Id_t id) const {
	Potassco::Id_t xId = Potassco::atom(Potassco::lit(id));
	Literal out = lit_false();
	if (validAtom(xId)) {
		out = getRootAtom(xId)->literal();
	}
	else if (xId >= bodyId) {
		CLASP_FAIL_IF(!validBody(xId - bodyId), "Invalid condition");
		out = getBody(getEqBody(xId - bodyId))->literal();
	}
	return out ^ (Potassco::lit(id) < 0);
}
...
void LogicProgram::prepareProgram(bool checkSccs) {
	...
	addTheory();
	uint32 nAtoms  = (uint32)atoms_.size();
	if (incData_) { incData_->atoms.hi = nAtoms; }
	...
	setFrozen(true);
	Preprocessor p;
	if (hasConflict() || !propagate(true) || !p.preprocess(*this, opts_.iters != 0 ? Preprocessor::full_eq : Preprocessor::no_eq, opts_.iters, opts_.dfOrder)) {
		setConflict();
		return;
	}
...
void LogicProgram::addTheory() {
	if (theory_) {
		PrgBody* supp = 0;
		for (TheoryData::atom_iterator it = theory_->currBegin(), end = theory_->end(); it != end; ++it) {
			const Potassco::TheoryAtom& a = **it;
			PrgAtom* prgAt = a.atom() && a.occurrence() == Potassco::TheoryAtom::occ_body ? resize(a.atom()) : 0;
			if (prgAt && isNew(a.atom()) && prgAt->supports() == 0) {
				if (!supp) { supp = getTrueBody(); }
				supp->addHead(prgAt, PrgEdge::GammaChoice);
			}
		}
	}
}
bool LogicProgram::TFilter::operator()(const Potassco::TheoryAtom& a) const {
	bool body = a.occurrence() == Potassco::TheoryAtom::occ_body;
	Atom_t id = a.atom();
	if (body || (self->getLiteral(id) != lit_false() && self->getRootAtom(id)->value() != value_false)) {
...
bool LogicProgram::addConstraints() {
	...
	for (AtomList::const_iterator it = atoms_.begin()+startAtom(), end = atoms_.end(); it != end; ++it) {
		if (!toConstraint(*it, *this, gc)) { return false; }
	...
	for (DomRules::iterator it = (j = doms.begin()), end = doms.end(); it != end; ++it) {
		...
		if (!atomState_.isSet(it->atom, AtomState::dom_flag)) {
			if (!ctx()->marked(posLit(svar))) {
				...
				Eq n = { ctx()->addVar(Var_t::Atom), slit };
				eqVec.push_back(n);
				svar = n.var;
				slit = posLit(svar);
...
bool LogicProgram::simplifyBody(const BodyView& body, BodyData& out, BodyData::Meta* mOut) {
	...
	sBody.reserve(body.lits.size);
	enum { BOT = -1 };
	int bt = body.type;
	weight_t bound = body.type != Body_t::Normal ? body.bound : static_cast<weight_t>(body.lits.size);
	BodyData::Meta meta;
	weight_t maxW  = 1;
...
bool LogicProgram::extractCondition(Id_t id, Potassco::LitVec& out) const {
	...
	if (!id || Potassco::atom(Potassco::lit(id)) < bodyId) {
...
LogicProgramVisitor::~LogicProgramVisitor() {}
#undef RT

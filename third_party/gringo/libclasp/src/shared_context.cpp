...
ConstString::ConstString(const StrView& str) : str_(mkstring(str.first, str.size)) {}
...
// OutputTable
/////////////////////////////////////////////////////////////////////////////////////////
OutputTable::OutputTable() : theory(0), hide_(0) {}
...
void OutputTable::add(const Range32& r) {
	if (r.hi > r.lo) {
		CLASP_ASSERT_CONTRACT(vars_.empty() || r.lo >= vars_.back().hi);
		vars_.push_back(r);
	}
}
...
uint32 OutputTable::size() const {
	uint32 r = numFacts() + numPreds();
	for (range_iterator it = vars_.begin(), end = vars_.end(); it != end; ++it) {
		r += static_cast<uint32>(it->hi - it->lo);
	}
	return r;
}
...
SharedContext::SharedContext() 
	...
	setFrozen(addVar(Var_t::Atom), true);
	stats_.vars.num = 0;
...
Var SharedContext::addVar(const VarInfo& nv) {

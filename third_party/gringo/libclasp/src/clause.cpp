...
ClauseCreator::Result ClauseCreator::integrate(Solver& s, SharedLiterals* clause, uint32 modeFlags, ConstraintType t) {
	...
	if ((xs & (status_unit|status_unsat)) != 0) {
		...
		if (result.local && (modeFlags & clause_int_lbd) != 0 && result.status != status_unsat) {
			result.local->lbd(s.updateLearnt(lit_false(), clause->begin(), clause->end(), result.local->lbd(), true));
		}
	}
	return result;
}
...
Clause::Clause(Solver& s, const ClauseRep& rep, uint32 tail, bool extend) 
	...
	if (!isSmall()) {
		...
		data_.lits[0] = rep.size > ClauseHead::HEAD_LITS   ? rep.lits[ClauseHead::HEAD_LITS].rep()   : lit_false().rep();
		data_.lits[1] = rep.size > ClauseHead::HEAD_LITS+1 ? rep.lits[ClauseHead::HEAD_LITS+1].rep() : lit_false().rep();
...
void Clause::undoLevel(Solver& s) {
	...
	Literal* r = head_+t;
	while (!r->flagged() && s.value(r->var()) == value_free) {
		++t;
		++r;
	}

...
#define CLASP_VERSION "3.2.0-RC-R49713-R10184"
#endif
...
public:
	...
	//! Adds t as an additional theory propagator to be attached to all solvers.
	void           addTheoryPropagator(TheoryPropagator* t, Ownership_t::Type ownership = Ownership_t::Retain);
...
private:
	struct ConfiguratorProxy {
		ConfiguratorProxy(Configurator* c, Ownership_t::Type ownership, bool once);
		bool addPost(Solver& s) const;
		void prepare(SharedContext&) const;
		void destroy();
		Configurator* ptr() const;
		uint64 ptr_;
		mutable uint64 map_;
	};
	typedef PodVector<ConfiguratorProxy>::type UserDepVec;
	void resetUser();
...
public:
	...
	bool               solved()              const;
	//! Returns whether the active step was interrupted.
	...
	*/
	...
	/*!
	 * \pre solving() is false and program updates are enabled (multishot() is true).
	 * \post !solved()
	 * \param updateConfig If true, the function applies any configuration changes.
	 * \param sig An action to be performed if the current step was interrupted via
	 *            a call to interrupt(). The default action throws an instance of std::logic_error.
	 */
	ProgramBuilder&    update(bool updateConfig = false, void (*sig)(int) = 0);
private:

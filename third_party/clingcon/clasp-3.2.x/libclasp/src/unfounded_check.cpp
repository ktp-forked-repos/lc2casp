...
void DefaultUnfoundedCheck::createLoopFormula() {
	...
	if (loopAtoms_.size() == 1) {
		...
		solver_->addLearnt(static_cast<LearntConstraint*>(ante.constraint()), uint32(loopAtoms_.size() + activeClause_.size()), Constraint_t::Loop);
	}
	do {
...
void DefaultUnfoundedCheck::computeReason(UfsType t) {
	...
	pickedExt_.clear();
	info_     = ClauseInfo(Constraint_t::Loop);

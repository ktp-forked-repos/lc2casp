...
uint32 RuleTransform::transform(const RuleView& r, Strategy s) {
	...
	else if (r.head.atoms.size > static_cast<uint32>(r.head.type == Head_t::Disjunctive)) {
		impl_->body_.reset();
		uint32 nAux = (r.body.lits.size > 1) && (r.head.atoms.size > 1);
		if (nAux) {
			...
			impl_->body_.lits.assign(Potassco::begin(r.body.lits), Potassco::end(r.body.lits));
		}
		return nAux + (r.head.type == Head_t::Choice ? impl_->transformChoice(r.head.atoms) : impl_->transformDisjunction(r.head.atoms));
	}
...
uint32 RuleTransform::Impl::transform(const RuleView& r, Strategy s) {
	...
	body_.lits.assign(Potassco::begin(r.body.lits), Potassco::end(r.body.lits));
	...
	Atom_t h = !Potassco::empty(r.head.atoms) ? *Potassco::begin(r.head.atoms) : 0;
	bool aux = r.head.type == Head_t::Choice || r.head.atoms.size > 1;
	if (aux) {
		h = newAtom();
...
uint32 RuleTransform::Impl::addRule(Atom_t head, bool add, uint32 bIdx, weight_t bound) {
	...
	BodyView b = {Body_t::Normal, Potassco::Body_t::BOUND_NONE, Potassco::toSpan<Potassco::WeightLit_t>()};
	if (bound <= 0) {
		assert(add);
		b.lits = Potassco::toSpan(&body_.lits[bIdx], 1);
		return addRule(head, b);
	}
	if ((maxW - minW) < bound) {
		// remaining literals are all needed to satisfy bound
		bIdx += static_cast<uint32>(!add);
		b.lits = Potassco::toSpan(&body_.lits[bIdx], body_.lits.size() - bIdx);
		return maxW >= bound ? addRule(head, b) : 0;
	}
	Potassco::WeightLit_t lits[2] = { body_.lits[bIdx], {Potassco::lit(getAuxVar(bIdx + 1, bound)), 1} };
	uint32 first = static_cast<uint32>(!add);
	b.lits = Potassco::toSpan(lits + first, 2 - first);
	return addRule(head, b);
}
/////////////////////////////////////////////////////////////////////////////////////////
// class SccChecker
...
void PrgHead::assignVar(LogicProgram& prg, PrgEdge support, bool allowEq) {
	...
	if (numS == 0 && support == PrgEdge::noEdge()) {
		...
		if (support.isNormal() && sup->hasVar() && (!newVar || sup->value() == value_true)) {
			...
			setLiteral(posLit(prg.ctx()->addVar(Var_t::Atom)));
		}
	}
}
...
PrgBody::PrgBody(LogicProgram& prg, const BodyData::Meta& meta, const BodyView& body, bool addDeps)
	...
	, size_(body.lits.size), extHead_(0), type_(body.type), sBody_(0), sHead_(0), freeze_(0), unsupp_(0) {
	...
	if (W) {
		data_.ext[0] = SumExtra::create(body.lits.size);
	}
	weight_t   w   = 1;
...
PrgBody* PrgBody::create(LogicProgram& prg, const BodyData::Meta& meta, const BodyView& body, bool addDeps) {
	uint32 bytes = sizeof(PrgBody) + (body.lits.size * sizeof(Literal));
...
void PrgBody::assignVar(LogicProgram& prg) {
	...
	if (size == 0 || value() == value_true) {
		...
		setLiteral(posLit(prg.ctx()->addVar(Var_t::Body)));	
	}
	else {

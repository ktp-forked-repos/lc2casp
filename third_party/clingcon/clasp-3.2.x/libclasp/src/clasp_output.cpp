// 
// Copyright (c) 2009-2015, Benjamin Kaufmann
// 
...
void TextOutput::printSolveProgress(const Event& ev) {
	...
	if      (const BasicSolveEvent* be = event_cast<BasicSolveEvent>(ev)) { Clasp::Cli::format(*be, line, 128); }
		...
		const char* out = bnd->upper != INT64_MAX 
				? clasp_format(line, 128, "%" PRId64 ";%" PRId64, bnd->lower, bnd->upper)
				: clasp_format(line, 128, "%" PRId64 ";*", bnd->lower);
		printLN(cat_comment, "Bounds@%u: [%s]", bnd->at, out);
		return;
	}
	else                                                                  { return; }

...
SmodelsInput::SmodelsInput(LpElement& out, const Options& opts, AtomTable* syms) : out_(out), atoms_(syms), nodes_(0), opts_(opts), delSyms_(false) {}
...
static BodyView matchBody(BufferedStream& str, std::vector<WeightLit_t>& lits, Body_t type) {
	...
	BodyView out = {type, bound, toSpan(lits)};
	return out;
}
		...
		switch (rt) {
			...
			case Basic:
				if (Atom_t a = matchAtom()) {
					...
					HeadView head ={Head_t::Disjunctive, toSpan(&a, 1)};
					out_.rule(head, body);
				}
				break;
			...
			case Choice:      // n a1..an normal body
				if (unsigned n = matchPos()) {
					...
					HeadView head ={rt == Choice ? Head_t::Choice : Head_t::Disjunctive, toSpan(atoms)};
					out_.rule(head, body);
				}
...
bool SmodelsInput::readCompute(const char* comp, bool val) {
	...
	WeightLit_t w ={0, 1};
	HeadView empty ={Head_t::Disjunctive, toSpan<Atom_t>()};
	BodyView unit  ={Body_t::Normal, 1, toSpan(&w, 1)};
	while ((w.lit = (Lit_t)matchPos()) != 0) {
		if (val) { w.lit = -w.lit; }
		out_.rule(empty, unit);
	}
	return true;
}
...
int readSmodels(std::istream& in, LpElement& out, ErrorHandler err, const SmodelsInput::Options& opts) {
...
SmodelsOutput& SmodelsOutput::add(int rt, const WeightLitSpan& body, unsigned bnd) {
	...
	for (const WeightLit_t* x = begin(body); x != end(body); ++x) { 
		neg += static_cast<unsigned>(lit(*x) < 0);
	}
	...
	for (unsigned i = 0, k; i != 2; ++i) {
		if ((k = n[i]) == 0) { continue; }
		for (const WeightLit_t* x = begin(body); x != end(body); ++x) { 
			if (unsigned(lit(*x) > 0) == i) { add(atom(*x)); if (++k == n[i]) break; }
		}
	}
	if (rt == Weight || rt == Optimize) {
		for (unsigned i = 0, k; i != 2; ++i) {
			if ((k = n[i]) == 0) { continue; }
			for (const WeightLit_t* x = begin(body); x != end(body); ++x) { 
				if (unsigned(lit(*x) > 0) == i) { 
					require(weight(*x) > 0, "negative weight not supported");
					add((unsigned)weight(*x));
					if (++k == n[i]) break;
				}
			}
		}
...
void SmodelsOutput::rule(const HeadView& head, const BodyView& body) {
	...
	startRule(rt).add(head).add(rt, body.lits, static_cast<unsigned>(body.bound)).endRule();
}
...
void SmodelsOutput::output(const StringSpan& str, const LitSpan& cond) {
	...
	os_ << unsigned(*cond.first) << " ";
	os_.write(begin(str), str.size);
	os_ << "\n";
}

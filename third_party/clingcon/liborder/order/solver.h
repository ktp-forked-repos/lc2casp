...
public:
    ...
    void flag() { store_set_bit(rep_, 0); }

    //! Clears the flag of this literal.
    void clearFlag() { store_clear_bit(rep_, 0); }

    //! Returns true if the flag of this literal is set.
    bool flagged() const { return test_bit(rep_, 0); }
...
{
public:
    virtual bool isTrue(const Literal&) const = 0;
    virtual bool isFalse(const Literal&) const = 0;
    virtual bool isUnknown(const Literal&) const = 0;
...
public:
    ...
    /// is allowed to create free spare variables, remove them with makeRestFalse
    /// once finished creating variables

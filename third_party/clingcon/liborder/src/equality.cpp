...
namespace order
{
    ...
    bool EqualityProcessor::process(std::vector<ReifiedLinearConstraint>& linearConstraints)
    {
        ...
        for (unsigned int i = 0; i < linearConstraints.size();)
        {
            ...
            if ((s_.isTrue(rc.v) && rc.l.getRelation()==LinearConstraint::Relation::EQ) || (s_.isFalse(rc.v) && rc.l.getRelation()==LinearConstraint::Relation::NE))
            {
        ...
        for (auto& i : equals)
            linearConstraints.emplace_back(ReifiedLinearConstraint(std::move(i),s_.trueLit()));

...
bool LinearConstraint::productOfDomainsExceptLastLEx(const VariableCreator& vc, uint64 x) const
{
    assert(views_.size()!=0);
    assert(normalized_);
    uint64 ret = 1;
...
std::vector<LinearConstraint> LinearConstraint::split(const CreatingSolver& s, VariableCreator& vc, const Config& conf, TruthValue t) const
{
    ...
    l.sort(vc);
    if (l.views_.size() <= conf.splitsize_maxClauseSize.first || l.productOfDomainsExceptLastLEx(vc,conf.splitsize_maxClauseSize.second))
    {
        ret.emplace_back(l);
        return ret;
    }
    assert(r_== Relation::EQ || r_ == Relation::LE || r_ == Relation::GE);
...
std::vector<ReifiedLinearConstraint> ReifiedLinearConstraint::split(VariableCreator& vc, CreatingSolver& s, const Config& conf) const
{
    ...
    ret.emplace_back(std::move(*splitted.begin()),v);
    for (auto i = splitted.begin()+1; i != splitted.end(); ++i)
    {
        ret.emplace_back(std::move(*i),s.trueLit()); // original
    }
    return ret;
}

...
/////////////////////////////////////////////////////////////////////////////////////////
namespace Cli {
ClaspAppOptions::ClaspAppOptions() : outf(0), compute(0), ifs(' '), hideAux(false), onlyPre(false), printPort(false), outLbd(Activity::MAX_LBD), inLbd(Activity::MAX_LBD) {
...
void ClaspAppBase::validateOptions(const ProgramOptions::OptionContext&, const ProgramOptions::ParsedOptions& parsed, const ProgramOptions::ParsedValues& values) {
	...
	if (!app.lemmaIn.empty() && !isStdIn(app.lemmaIn) && !std::ifstream(app.lemmaIn.c_str()).is_open()) {
		throw Error("'lemma-in': could not open file!");
	}
...
void ClaspAppBase::shutdown() {
	...
	if (out_.get()) { out_->shutdown(result); out_ = 0; }
	if (!claspAppOpts_.lemmaOut.empty()) {
		...
		Constraint_t::Set x; x.addSet(Constraint_t::Conflict);
...
void ClaspAppBase::readLemmas(SharedContext& ctx) {
	...
	for (ClauseCreator clause(&s); file && ok; ) {
		...
		clause.start(Constraint_t::Conflict);
...
bool ClaspAppBase::handlePostGroundOptions(ProgramBuilder& prg) {
	if (!claspAppOpts_.onlyPre) { return true; }
	prg.endProgram();
...
bool WriteLemmas::unary(Literal p, Literal x) const {
	if (!isSentinel(x) && x.rep() > p.rep() && (p.flagged() + x.flagged()) != 0) {
		os_ << ~p << " " << x << " 0\n"; 
		++outShort_;
	}
	return true;
...
bool WriteLemmas::binary(Literal p, Literal x, Literal y) const {
	if (x.rep() > p.rep() && y.rep() > p.rep() && (p.flagged() + x.flagged() + y.flagged()) != 0) {
		os_ << ~p << " " << x << " " << y << " 0\n"; 
		++outShort_;
	}
	return true;
